...
interface ProjectListSortModalProps {
  isOpen: boolean;
  onClose: () => void;
  value: ProjectListSort[];
  onApply: (sort: ProjectListSort[]) => void;
}

function sortLabel(key: ProjectListSortKey): string {
  return PROJECT_LIST_SORT_OPTIONS.find((o) => o.key === key)?.label ?? key;
}

function not(a: readonly ProjectListSortKey[], b: readonly ProjectListSortKey[]) {
  return a.filter((value) => !b.includes(value));
}

function intersection(a: readonly ProjectListSortKey[], b: readonly ProjectListSortKey[]) {
  return a.filter((value) => b.includes(value));
}

function union(a: readonly ProjectListSortKey[], b: readonly ProjectListSortKey[]) {
  return [...a, ...not(b, a)];
}

export default function ProjectListSortModal({
  isOpen,
  onClose,
  value,
  onApply,
}: ProjectListSortModalProps) {
  const allKeys = useMemo(() => PROJECT_LIST_SORT_OPTIONS.map((o) => o.key), []);

  const [chosen, setChosen] = useState<ProjectListSort[]>([...DEFAULT_PROJECT_LIST_SORT]);
  const [checkedLeft, setCheckedLeft] = useState<ProjectListSortKey[]>([]);
  const [checkedRight, setCheckedRight] = useState<ProjectListSortKey[]>([]);
  ...
  useEffect(() => {
    ...
    const next: ProjectListSort[] =
      value.length > 0
        ? value.map((s) =>
            isOptionalSortKey(s.key)
              ? {
                  ...s,
                  emptyPlacement: (s.emptyPlacement === 'first' ? 'first' : 'last') as ProjectListEmptyPlacement,
                }
  ...
  const handleToggle =
    (list: 'left' | 'right', key: ProjectListSortKey) =>
    () => {
      if (list === 'left') {
        setCheckedLeft((prev) =>
  ...
  const moveCheckedToRight = () => {
    ...
    setChosen((prev) => [...prev, ...moving.map((key) => createSortEntry(key))]);
  ...
  const moveAllToRight = () => {
    setChosen((prev) => [...prev, ...leftKeys.map((key) => createSortEntry(key))]);
    setCheckedLeft([]);
  };
  ...
  const setDirection = (key: ProjectListSortKey, direction: ProjectListSortDirection) => {
    setChosen((prev) => prev.map((s) => (s.key === key ? { ...s, direction } : s)));
  };

  const setEmptyPlacement = (key: ProjectListSortKey, emptyPlacement: ProjectListEmptyPlacement) => {
    setChosen((prev) => prev.map((s) => (s.key === key ? { ...s, emptyPlacement } : s)));
  };

  const movePriority = (key: ProjectListSortKey, delta: -1 | 1) => {
    setChosen((prev) => {
  ...
  const handleApply = () => {
    onApply(chosen.length > 0 ? chosen : [...DEFAULT_PROJECT_LIST_SORT]);
    onClose();
  };
  ...
  return (
    ...
    >
      <p className="text-sm text-gray-600 mb-4">
        左側から並び替えに使う項目を選び、右側で優先順位と昇順・降順を設定します。右側の上にある項目ほど優先されます。子プロジェクトは親の配下にまとめたまま、兄弟間で同じ条件で並び替えます。
      </p>
