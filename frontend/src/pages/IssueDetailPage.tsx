...
export default function IssueDetailPage({ user }: Props) {
  ...
  const load = () => api.get(`/issues/${id}`).then((res) => setIssue(res.data));
  ...
  const handleComment = async (e: FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.post(`/issues/${id}/comments`, { content: comment });
      setComment('');
      load();
    } finally {
      setSubmitting(false);
  ...
  const handleSaveEditComment = async (commentId: number) => {
    ...
    try {
      await api.put(`/issues/${id}/comments/${commentId}`, { content: editCommentContent });
  ...
  const handleAddRelation = async (e: FormEvent) => {
    ...
    try {
      await api.post(`/issues/${id}/relations`, { issueToId: Number(relationIssueId), relationType: 'precedes' });
      setRelationIssueId('');
  ...
  const toggleAddRelation = async () => {
    if (!showAddRelation) {
      ...
      try {
        ...
        const filtered = issuesArray.filter((i: Issue) => i.id !== Number(id));
    ...
    <div>
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        <div className="flex items-start justify-between mb-4">
          ...
          <Link to={`/issues/${id}/edit`} title="編集" className="p-1.5 text-sky-600 hover:bg-sky-50 rounded">
      ...
      <div className="bg-white rounded-lg shadow p-5 mb-6">
        ...
        {showAddRelation && (
          <form onSubmit={handleAddRelation} className="mb-4 space-y-2 bg-slate-50 p-3 rounded border border-slate-100">
            ...
            {!relationIssueId && !isAddingRelation && (
              <p className="text-[10px] text-red-400 text-right mt-1">チケットを選択してください</p>
            )}
          </form>
        )}
