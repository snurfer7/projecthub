...
| `/issues/:id` | IssueDetailPage | チケット詳細。説明は Markdown 表示。親チケットへのリンクを表示。子がある場合の開始・終了・ステータスは集約値 |
