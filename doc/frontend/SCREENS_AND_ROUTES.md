...
| `/projects/:projectId/issues/new` | IssueFormPage | チケット新規作成。親チケット選択可（`projects.issues.fields.parent`）。**担当者は複数ユーザー＋担当グループ（単一）を選択可**（`projects.issues.fields.assignee`）。**新規作成時の担当者初期値はログインユーザー**（担当候補 `meta.users` に含まれる場合のみ。編集時は既存値を維持）。**新規作成時の終了日初期値は開始日と同じ日付**（時刻はシステム終了時刻。`initialEndDate` 指定時はその値を優先）。ステータス選択肢はロールの利用可能ステータス（`meta.workflow.assignableStatusIds`）に制限。子を持つ編集時はステータス・開始日・開始時刻・終了日・終了時刻は入力不可で子孫集約値を表示（ステータスは position 最小）。保存: `projects.issues` canInput |
