...
| `/projects/:projectId/gantt` | GanttPage | ガント（プロジェクト単位）。親チケットは子孫の期間を集約表示しバーの移動・リサイズ不可。子はインデント表示。開始日・終了日・期日がすべて未設定のチケットも行として表示（バーはなし） |
