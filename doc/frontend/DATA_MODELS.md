...
- **IssueFilterCriteria** — `trackerIds`, `statusIds`, `assignedToIds`, `assignedToGroupIds`, `assignedToGroupMemberIds`, `includeUnassigned`（true のとき担当ユーザー・担当グループのいずれも未設定のチケットを担当者条件の OR 対象に含める。省略時は false）, `dueDateStart`, `dueDateEnd`, `dueDateMode`, `dueDateRelative`（チケット期限。ガント／カンバン等）, `scheduleDateStart`, `scheduleDateEnd`, `scheduleDateMode`, `scheduleDateRelative`（チケット開始日〜終了日との期間重なり。主に時間タブ）。相対指定時は対応する `*Mode`=`relative` と `*Relative` プリセットを保持し、絞り込み時に日付範囲を再計算する。
