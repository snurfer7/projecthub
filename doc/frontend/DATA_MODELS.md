...
- **Activity** — CRM アクティビティ（`type`, subject, description, dueDate, completed, `locationId`・`location`, `contactId`・`contact`, `assignedToId`・`assignedTo`, `dealId`・`deal`, **`projects`**（`{ id, name, identifier }[]`、N:N）, **`fileCommentId`**, **`fileComment`**＝`{ id, attachments: Attachment[] } | null` 等）。`type` の標準値は [API_SPEC.md](../backend/API_SPEC.md) の Activity.type（`call`〜`claim`）を参照。
...
- **Tracker**, **IssuePriority** — マスタ（id, name, position 等）。
- **IssueStatus** — マスタ（id, name, **isClosed**（終了フラグ）, position）。
- **Role** — id, name, position, isDefaultRole（メンバー追加時のロール選択初期値・複数可）, statuses, transitions, permissions（プロジェクト詳細の RolePermission）。
...
- **ProjectFilterCriteria** — `searchQuery`, `dueDateStart`, `dueDateEnd`, `dueDateMode`, `dueDateRelative`, `companyIds`, `statuses`, `memberIds`, `memberGroupIds`, `memberGroupMemberIds`（メンバー絞り込み。ユーザー ID・グループ ID・グループ選択時の展開メンバー ID。グループ選択時はプロジェクトへのグループ割当および所属ユーザーのメンバー所属を OR で対象に含む）。
- **IssueFilterCriteria** — `trackerIds`, `statusIds`, `assignedToIds`, `assignedToGroupIds`, `assignedToGroupMemberIds`（後方互換用・常に空。グループ選択時はメンバー展開しない）, `includeUnassigned`（true のとき担当ユーザー・担当グループのいずれも未設定のチケットを担当者条件の OR 対象に含める。省略時は false）, `dueDateStart`, `dueDateEnd`, `dueDateMode`, `dueDateRelative`（チケット期限。ガント／カンバン等）, `scheduleDateStart`, `scheduleDateEnd`, `scheduleDateMode`, `scheduleDateRelative`（チケット開始日〜終了日との期間重なり。ガント／カンバン／時間）, `includeUnscheduled`（true のとき開始・終了期間指定ありでも `startDate`・`endDate` とも未設定のチケットを期間重なりと OR で含める。省略時は false。片方のみ設定は点として期間判定し、未設定扱いしない）。担当者条件は `assignedToIds`（ユーザー担当）・`assignedToGroupIds`（担当グループ一致のみ）・`includeUnassigned` を OR。グループ選択時は所属メンバーのユーザー担当チケットは含めない。ガント／カンバン／時間ではトラッカー・ステータス・担当者を API Query（`assignedToIds` と `assignedToGroupIds` はサーバー側 OR）で絞り込み、期日・開始終了・未割当含む場合の担当者 OR はクライアント側。相対指定時は対応する `*Mode`=`relative` と `*Relative` プリセットを保持し、絞り込み時に日付範囲を再計算する。

## 保存済み検索

- **SavedSearch** — id, userId, viewMode（`list` | `gantt` | `kanban` | `time`）, name, isDefault, filter（`SavedSearchFilter`）, createdAt, updatedAt。
- **SavedSearchFilter** — `projectFilter?`（ProjectFilterCriteria）, `issueFilter?`（IssueFilterCriteria）, `ganttZoom?`（`day`|`month`|`year`）, `showEmptyProjects?`（boolean。ガントでチケットなしプロジェクトを表示するか。省略時は true）, `ganttStartValue?`, `ganttEndValue?`, `timeRecordStartDate?`, `timeRecordEndDate?`, `timeRecordDateMode?`, `timeRecordDateRelative?`, `timeRecordFilterUserIds?`（(number|string)[]。ユーザー ID およびグループ選択 `g:{groupId}`。API 呼び出し時にグループはメンバーのユーザー ID へ展開）, `listSort?`（`{ key, direction, emptyPlacement? }[]`。一覧／ガント／時間のプロジェクト複合並び替え。`key` は `companyName`|`name`|`identifier`|`dueDate`|`issueCount`|`status`、`direction` は `asc`|`desc`、`emptyPlacement` は省略可能な項目向けに `first`|`last`（未設定値の位置。省略時は末尾）。省略時は適用しない）、`issueSort?`（`{ key, direction, emptyPlacement? }[]`。ガント／カンバン／時間のチケット複合並び替え。`key` は `id`|`subject`|`tracker`|`status`|`priority`|`dueDate`|`startDate`|`endDate`|`assignee`、`emptyPlacement` は省略可能な項目（期日・開始日・終了日・担当者）向け。省略時は適用しない）。日付範囲条件の保存: **相対指定**では `*Mode` / `*Relative` のみを保存し開始・終了日付は保存しない（適用時に再計算）。**直接指定**では `*Mode`=`direct` とともに開始・終了日付も保存する。
