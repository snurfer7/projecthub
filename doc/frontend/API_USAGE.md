...
- 企業一覧ページは `api.get('companies', { params: { page, pageSize, q, isSales, isPurchase } })` でページング応答（`items`, `total`, …）を取得する。`isSales` / `isPurchase` はフィルタ ON のとき `true` を付ける。ドロップダウン用など全件が必要な箇所は `page` なしで配列を取得する。登録・更新 Body に `isSales` / `isPurchase`（boolean）を含める（field 権限 `companies.fields.transactionTypes`）。
- 連絡先一覧ページは `api.get('crm/contacts', { params: { page, pageSize, q } })` でページング応答を取得する。企業別の連絡先タブは `page` なしで `api.get('crm/contacts', { params: { companyId } })` を使用する（配列）。CSV 出力時は一覧と同じ `q` を付け、`pageSize=100` で一致する全ページを取得し、拠点の郵便番号・住所および所属企業の取引区分（`company.isSales` / `company.isPurchase`）を含む列を出力する。
