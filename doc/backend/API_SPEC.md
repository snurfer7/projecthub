...
| POST | `/` | プロジェクト作成。権限: `projects` input。Body: `name`・`identifier`（必須）、`description`・`companyId`・`locationId`・`contactId`・`parentId`・`dueDate`・`remarks`・`relatedCompanies`（任意）。`locationId` / `contactId` を指定する場合は `companyId` が必須で、いずれも当該企業配下であること（不一致時 400）。任意の **`sourceActivityId`**: 指定時は活動の存在確認のうえ、活動の `companyId` が作成プロジェクトの主企業または `relatedCompanies` のいずれかと一致することを検証（`companyId` 未指定なら活動の企業を主企業に採用）。`description` / `locationId` / `dueDate` が Body で未指定のときは活動の詳細・拠点・期限日を採用する。トランザクション内でプロジェクト作成と `ActivityProject` 紐づけを行う。紐づけに `companies.activities` input は不要 |
...
| PUT | `/:id` | チケット更新（所属プロジェクトのみ）。Body: **`projectId`**（任意・変更時はプロジェクト移動）, **`assignedToIds`**（指定時は中間テーブルをその集合に同期。空配列で全解除。未指定時は変更しない。旧 `assignedToId` も受理）, `assignedToGroupId`, `parentId`（任意・null で解除）, **`estimatedHours`**（任意・0 以上・**0.5 刻み**。空／null で未設定）, **`statusId`**, **`doneRatio`** 等。子チケットがある場合 `startDate` / `endDate` / `statusId` の更新は 400。`statusId` 変更時はロールの利用可能ステータス＋ステータス遷移を検証（否則 400）。**`projectId` 変更時**: 当該チケット＋全子孫を移動先へ一括更新し、移動対象ルートの `parentId` は解除（子孫の親子は維持）。紐づく `TimeEntry.projectId` も同トランザクションで同期。移動元・移動先双方で `projects.issues` input が必要。field `projects.issues.fields.project`（値が変わるとき）。移動に伴う親解除は `fields.parent` を要求しない。権限: `projects.issues.fields.project` / `projects.issues.fields.status` / `projects.issues.fields.doneRatio` / `projects.issues.fields.assignee` / `projects.issues.fields.parent` / `projects.issues.fields.estimatedHours` 等 |
...
- **認証エラー**: トークン未付与・無効・期限切れは **401**（`認証が必要です` / `トークンが無効です`）。クライアントは 401 時にログアウト・ログイン画面へ遷移すること。権限不足は **403**（認証済みだが操作不可）
